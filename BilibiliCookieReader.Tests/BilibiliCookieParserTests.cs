...
public class BilibiliCookieParserTests
{
    ...
    public void Prefers_bilibili_com_over_other_hosts()
    {
        var text = """
            ...
            .bilibili.com	TRUE	/	TRUE	1999999999	buvid3	buvid3-value
            """;
        ...
        Assert.Equal("buvid3-value", set.Buvid3.Value);
        ...
        Assert.Contains("buvid3=buvid3-value", set.ToCookieHeader());
        Assert.Contains("BUVID3=buvid3-value", set.ToGitHubSecretsBlock());
    }
    ...
    public void Parses_cookie_header_string()
    {
        var set = BilibiliCookieParser.ParseText(
            "SESSDATA=aaa; bili_jct=bbb; DedeUserID=123; buvid3=device-id; other=ignore");

        Assert.Equal("aaa", set.SessData.Value);
        Assert.Equal("bbb", set.BiliJct.Value);
        Assert.Equal("123", set.DedeUserId.Value);
        Assert.Equal("device-id", set.Buvid3.Value);
        Assert.Equal("SESSDATA=aaa; bili_jct=bbb; DedeUserID=123; buvid3=device-id", set.ToCookieHeader());
    }
    ...
    public void Parses_json_object_and_array()
    {
        var objectSet = BilibiliCookieParser.ParseText(
            """{"SESSDATA":"s1","BILI_JCT":"j1","DEDEUSERID":"u1","buvid3":"b1"}""");
        ...
        Assert.Equal("b1", objectSet.Buvid3.Value);
